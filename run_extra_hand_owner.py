"""Paired screen for the additional-worker MELON owner probe."""
...
CANDIDATE = "agents/autonomous_next/extra_hand_melon_owner_v1.py"
...
def _summary(rows):
    ...
    return {
        ...
        "hire_requests": sum(int(r["candidate"].get("telemetry", {}).get("hire_requests", 0)) for r in valid),
        "hire_days": sum(int(r["candidate"].get("telemetry", {}).get("hire_days", 0)) for r in valid),
        "extra_confirmed": sum(int(r["candidate"].get("telemetry", {}).get("extra_confirmed", 0)) for r in valid),
...
def main():
    ...
    parser.add_argument("--seeds", nargs="+", type=int, default=[57800, 57801, 57802, 57803])
    parser.add_argument("--output", default="experiments/extra_hand_melon_owner_screen.json")
    ...
    payload = {
        "schema_version": 1,
        "design": "reserved NE melon tile with an additional daily worker versus frozen portfolio",
        "baseline": BASELINE,
        "candidate": CANDIDATE,
