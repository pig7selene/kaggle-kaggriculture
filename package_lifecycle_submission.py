"""Mechanically embed lifecycle_ready_chain_v1 and its dependency graph."""
...
ENTRYPOINT = "agents/lifecycle_lc_combined.py"
SOURCES = (
...
def _render():
    lines = [
        '"""Standalone Kaggriculture agent: lifecycle_ready_chain_v1.',
        "",
        ...
        "",
        "",
        "_EMBEDDED_SOURCE_SHA256 = {",
    ]
    for path in SOURCES:
        lines.append(f"    {path!r}: {_sha256(path)!r},")
    lines.extend(["}", "", "", "_EMBEDDED_SOURCES = {"])
    for path in SOURCES:
