...
def _adv_apply(obs,action):
    ...
    # Demand-preserving extension: keep the parent's 3-turn timing with >=2 bakeries.
    adv_look=3 if shops.count('BAKERY')>=2 else _ADV_LOOK
    for off in range(1,adv_look+1):
        t=step+off
        if t>718:break
        for o in _adv_future(player,t):
            if not o or len(o)<3:continue
            if first is None:first=o
            if off>=5 and o[0]=='SELL' and o[1]=='WOOL' and 'YARN_STORE' in shops:continue
    ...
    if protected is not None and _e346_funded(obs,action,adv_look):
        protected=None
        _E346_REPORT['released']+=1
...
# Production-loader hardening: expose the intended final policy under the canonical name.
globals().pop('agent', None)
def agent(observation, configuration=None):
    return _e335_agent(observation, configuration)

# EXP342 parallel V49: demand-preserving timing by tetsutani, Apache-2.0.

# EXP342: absorb weed-clearing delay using a scheduled PASS, before all safety layers.
# Original integration by Ahmed Berat Ozer; weed recovery concept: tetsutani and salemali7 (Apache-2.0).
# Unlike fixed-length postprocessing, never discard a productive tape command to catch up.
_E342_REPAIR_REPORT=dict(events=0,replayed=0,caught_up=0,no_slack=0,errors=0)
_E342_WEED_ORIGINAL=Chassis._weed_repair

def _e342_units(row):
    return [row.get('farmer') or ['PASS'],*(row.get('hands') or [])]

def _e342_weed(self,action,view,st,route,step):
    active=st.setdefault('e342_slack',{})
    if step%24==0:active.clear()
    units=_e342_units(action)
    tape=self.routes[route]
    consumed=[]
    for k,tx in list(active.items()):
        if k>=len(units) or step>tx['end']:
            active.pop(k,None);continue
        previous=_e342_units(tape[step-1])
        units[k]=list(previous[k]) if k<len(previous) else ['PASS']
        st['pending'].pop(k,None)
        _E342_REPAIR_REPORT['replayed']+=1
        if step==tx['end']:
            consumed.append(k);_E342_REPAIR_REPORT['caught_up']+=1
    new=[]
    for k in range(min(len(units),len(view.positions))):
        if k in active:continue
        cmd=units[k]
        if not cmd or cmd[0] not in ('PLANT','BUILD_COOP','BUILD_PASTURE'):continue
        tile=_tile_at(view.tiles,view.positions[k])
        if not isinstance(tile,dict) or tile.get('kind')!='WEED':continue
        end=None
        for t in range(step+1,min((step//24+1)*24,len(tape))):
            future=_e342_units(tape[t])
            if k<len(future) and future[k]==['PASS']:
                end=t;break
        if end is None:
            _E342_REPAIR_REPORT['no_slack']+=1;continue
        active[k]={'end':end};new.append(k)
        _E342_REPAIR_REPORT['events']+=1
    action['farmer'],action['hands']=units[0],units[1:]
    _E342_WEED_ORIGINAL(self,action,view,st,route,step)
    for k in new+consumed:
        st['pending'].pop(k,None)
    for k in consumed:active.pop(k,None)

Chassis._weed_repair=_e342_weed

def _e342_agent(observation,configuration=None):
    return _e335_agent(observation,configuration)

# EXP346: release sale collateral only when purchases are already funded.
_E346_REPORT={'released':0,'errors':0}

def _e346_funded(obs,action,look):
    step=int(obs['step']);p=int(obs['player']);farm=obs['farms'][p]
    # Native route changes at shop revelations; do not budget across them.
    if any(t in (144,216,288,360,432,504,576,648) for t in range(step+1,min(719,step+look+1))):return False
    cost=1000
    hires=len(farm['hands'])
    prices={it:_r37_market_price(it,int(obs['market']['inventory'][it])-2000) for it in ('WHEAT','FERTILIZER')}
    sequences=[(step,action.get('market') or [])]+[(t,_adv_future(p,t)) for t in range(step+1,min(719,step+look+1))]
    day=step//24
    for t,orders in sequences:
        if t//24!=day:day=t//24;hires=0
        for o in orders:
            if not o:continue
            op=o[0]
            if op=='HIRE':cost+=_v219_fib(hires);hires+=1
                ...
                table=prices if op=='BUY_PRODUCT' else ({'GOOSE':300,'COW':400,'SHEEP':500} if op=='BUY_ANIMAL' else {'WHEAT':10,'CARROT':20,'TOMATO':50,'STRAWBERRY':100,'MELON':80})
                if o[1] not in table:return False
                cost+=max(0,int(o[2]))*table[o[1]]
    return float(farm['money'])>=cost

def _e346_agent(observation,configuration=None):
    if int(observation.get('step',0))==0:_E346_REPORT.update(released=0,errors=0)
    return _e342_agent(observation,configuration)

# ---------------------------------------------------------------- dynamic lead
import math as _dl_math
_DL_CFG = {'lookahead': 5, 'min_price': 50, 'min_ratio': 0.0, 'top_k': 0, 'always': ('WOOL', 'MILK', 'STRAWBERRY', 'MELON'), 'min_impact': 0.0, 'window_lo': 4, 'window_hi': 16}
_DL_BASEP = {"WHEAT": 25, "CARROT": 35, "TOMATO": 60, "STRAWBERRY": 120, "MELON": 250,
             "EGG": 50, "MILK": 160, "WOOL": 200, "FERTILIZER": 100}
# the online engine's curves: carrot, tomato and egg spike quadratically below I0
_DL_P = {"WHEAT": (25, 400, "sqrt", 0.80, "log", 0.20), "CARROT": (35, 450, "hinge", 1.00, "sqrt", 0.70),
...
_DL_HINGE = 8.0
_DL_PARENT = agent
_DL_TELEMETRY = {"steps": 0, "led_units": 0, "led_steps": 0, "no_route": 0, "errors": 0, "items": {}}
...
def _dl_shape(f, x, T):
    ...
    if f == "sq":
        return x * x
    if f == "sqrt":
        return _dl_math.sqrt(x)
    if f == "log":
        return _dl_math.log(1.0 + x)
    if f == "hinge":
        if not T or T <= 0:
            return x
        u = x / T
        return u + _DL_HINGE * max(0.0, u - 1.0) ** 2
    return x


def _dl_price(item, inv):
    base, T, bf, bt, af, at = _DL_P[item]
    if inv < 10000:
        amp = bt * base / _dl_shape(bf, T, T)
        p = base + amp * _dl_shape(bf, 10000 - inv, T)
    else:
        amp = at * base / _dl_shape(af, T, T)
        p = base - amp * _dl_shape(af, inv - 10000, T)
    return max(1, int(round(p)))


def _dl_worth_racing(item, inv, qty):
    """What the race is worth: this lot's revenue into the current book, minus
    its revenue if the clone's identical lot goes first. Steep curves -- wool,
    melon, and carrot or tomato when the town is short -- are where being second
    costs real money; a flat book is not worth a market slot."""
    if item not in _DL_P or qty <= 0:
        return 0.0
    first = sum(_dl_price(item, inv + j) for j in range(qty))
    second = sum(_dl_price(item, inv + qty + j) for j in range(qty))
    return first - second


def _dl_items(prices):
    cand = []
    for item, base in _DL_BASEP.items():
        p = int(prices.get(item, 0) or 0)
        if p < _DL_CFG["min_price"]:
            continue
        if _DL_CFG["min_ratio"] and p < _DL_CFG["min_ratio"] * base:
            continue
        cand.append((p, item))
    cand.sort(reverse=True)
    if _DL_CFG["top_k"]:
        cand = cand[: _DL_CFG["top_k"]]
    return {i for _, i in cand} | set(_DL_CFG["always"])


def agent(observation, configuration=None):
    action = _DL_PARENT(observation, configuration)
    try:
        _DL_TELEMETRY["steps"] += 1
        step = int(_dl_get(observation, "step", -1))
        lo = _DL_CFG.get("window_lo") or _DL_CFG["lookahead"]
        hi = _DL_CFG.get("window_hi") or _DL_CFG["lookahead"]
        if step < 0 or step + lo > 718:
            return action
        target = step + lo
        seat = int(_dl_get(observation, "player", 0) or 0)
        chassis = _IMPL.chassis
        st = chassis.players.get(seat) or {}
        tape = chassis.routes.get(st.get("route"))
        if not tape or target >= len(tape):
            _DL_TELEMETRY["no_route"] += 1
            return action
        shed = dict(_dl_get(_dl_get(observation, "private", {}) or {}, "shed", {}) or {})
        market_obs = _dl_get(observation, "market", {}) or {}
        prices = dict(_dl_get(market_obs, "prices", {}) or {})
        inv_all = dict(_dl_get(market_obs, "inventory", {}) or {})
        items = _dl_items(prices)
        market = [list(o) for o in (action.get("market") or []) if o][:10]
        planned_now = {}
        for o in market:
            if o[0] == "SELL" and len(o) >= 3:
                planned_now[o[1]] = planned_now.get(o[1], 0) + max(0, int(o[2]))
        # a window of future lots, not just one offset: depth rather than distance
        future = {}
        for t in range(step + lo, min(step + hi, 718) + 1):
            a_t = tape[t] if t < len(tape) and isinstance(tape[t], dict) else {}
            for o in a_t.get("market") or []:
        ...
        for item, fut_qty in future.items():
            ...
            if qty <= 0:
                continue
            if _DL_CFG["min_impact"] and _dl_worth_racing(item, int(inv_all.get(item, 10000)), qty) < _DL_CFG["min_impact"]:
                continue
            merged = False
            for o in market:
            ...
            if not merged:
                if len(market) >= 10:
                    continue
                market.append(["SELL", item, qty])
            planned_now[item] = planned_now.get(item, 0) + qty
            _DL_TELEMETRY["led_units"] += qty
            _DL_TELEMETRY["items"][item] = _DL_TELEMETRY["items"].get(item, 0) + qty
            led = True
        if led:
            _DL_TELEMETRY["led_steps"] += 1
            action["market"] = market
        return action
    except Exception:
        _DL_TELEMETRY["errors"] += 1
        return action


agent.telemetry = {**(getattr(_DL_PARENT, "telemetry", {}) or {}), "dynamic_lead": _DL_TELEMETRY}
kaggle_agent = agent


# ---------------------------------------------------------------- fertilizer programme
_FP_CFG = {'from_step': 144, 'max_price': 40, 'reserve': 0, 'carry': 3, 'top_up': False, 'value_ratio': 3.0}
...
_QC_CFG = {'from_step': 144, 'items': ('CARROT', 'TOMATO', 'STRAWBERRY', 'MELON', 'EGG', 'MILK', 'WOOL', 'WHEAT', 'FERTILIZER')}
